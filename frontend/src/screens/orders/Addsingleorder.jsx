...
/**
 * Addsingleorder Component: Handles the creation of new single orders.
 * It includes form handling, validation, and interaction with Firestore.
 */
function Addsingleorder() {
     ...
     * Sets the userId state variable.
     */
    useEffect(() => {
     ...
     * Queries Firestore for warehouses where the user_id matches the current user's ID.
     * Updates the warehouses state variable with the fetched data.
     * Depends on the userId state variable.
     */
    useEffect(() => {
        ...
        try { // Fetch pincode data from the API
            ...
            if (data[0].Status === 'Success') {
                ...
                setOrderData((prevState) => ({
                    // Update the city, state, and country based on the API response
                    ...prevState,
                    billing_city: data[0].PostOffice[0].Division,
                    billing_state: data[0].PostOffice[0].Circle,
    ...
    const generateOrderId = () => {
        ...
        // Update the order_id in the orderData state
    ...
    const handleFormChange = (event) => {
        ...
        // List of fields that should be treated as numeric
        ...
        // Convert the value to a number if it's a numeric field
    ...
    const createOrder = async () => {
        ...
        const orderItemData = getOrderItemsData();
        // Combine the order data with the order item data
        const finalOrderData = { ...orderData, order_items: orderItemData };

        // List of required fields for the order
        ...
        // Check if the billing address contains invalid characters
        ...
        if (isBillingAddressInvalid) {
            ...
            setToastMessage(errorMessage); // Set error message and show error toast
            setErrorToast(true);
            setSavingData(false);
            return;
        }
        ...
        // Set savingData to true to indicate that data is being saved
        setSavingData(true)
        ...
        await setDoc(doc(db, "orders", orderData.order_id), {
            ...
            logEvent(analytics, 'order_creation', {
                ...
                userId: userId,
                type: 'B2C',
            }); // Log the order creation event to Firebase Analytics
        }).catch((error) => {
            // Handle errors and log them
            setToastMessage('Error in creating order!');
            setErrorToast(true);
            // Reset savingData and generate a new order ID
            setSavingData(false);
            generateOrderId();
        })
    }
            ...
            <div className='mx-auto grid grid-cols-3 md:grid-cols-4 gap-4 mt-5'>
                <div class="mb-4">
                    ...
                    <input type="text" name='billing_customer_name'
                        ...
                        placeholder="" autoComplete='none' onChange={handleFormChange} required />
                </div>
                <div class="mb-4">
                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
